...
// DeferredPromise<void> that resolves when the original settles:
//
...
//     - whenResolved()          - Get void promise when settled (NON-consuming)
//
//   State Queries:
...
//     - reject(js, exception)   - Reject with kj::Exception, V8 value, or jsg::Value
...
// A continuation that receives the rejection reason
using CatchCallback = Function<void(Value)>;
...
class DeferredPromiseState final: public kj::Refcounted {
 public:
  ...
  struct Pending {
    ...
    kj::Vector<Function<void(bool /* succeeded */)>> resolutionObservers;
  ...
  struct Rejected {
    ...
    Value error;
    explicit Rejected(Value&& e): error(kj::mv(e)) {}
  };
  ...
  explicit DeferredPromiseState(DirectRejectedTag, Value&& error)
      : state(State::template create<Rejected>(kj::mv(error))) {}
    ...
    KJ_IF_SOME(pending, state.template tryGetUnsafe<Pending>()) {
      ...
      for (auto& observer: observers) {
        getContinuationQueue().schedule(
            js, [obs = kj::mv(observer)](Lock& js) mutable { obs(js, true /* succeeded */); });
      }
  ...
  void reject(Lock& js, Value error) {
    KJ_IF_SOME(pending, state.template tryGetUnsafe<Pending>()) {
      ...
      for (auto& observer: observers) {
        getContinuationQueue().schedule(
            js, [obs = kj::mv(observer)](Lock& js) mutable { obs(js, false /* succeeded */); });
      }

      // Notify JS resolver if one exists - error is forwarded to JS, go directly to Consumed
      ...
      KJ_IF_SOME(cb, callback) {
        // Pass error directly to continuation, skip storing in Rejected state
        // Note: forceTransitionTo needed because we're going Pending→Consumed, skipping Rejected
        state.template forceTransitionTo<Consumed>();
        getContinuationQueue().schedule(
            js, [c = kj::mv(cb), e = kj::mv(error)](Lock& js) mutable { c(js, kj::mv(e)); });
      } else {
        // No callback - store error in Rejected state for later consumption
        state.template transitionTo<Rejected>(kj::mv(error));
      }
    }
  }

  void reject(Lock& js, kj::Exception&& exception) {
    reject(js, js.exceptionToJs(kj::mv(exception)));
  }
...
class DeferredPromiseResolver {
 public:
  ...
  // Reject the promise with a JavaScript exception value.
  // Runs all attached error handlers synchronously.
  // Has no effect if the promise is already resolved or rejected.
  void reject(Lock& js, v8::Local<v8::Value> exception) {
    state->reject(js, Value(js.v8Isolate, exception));
  }

  // Reject the promise with a kj::Exception.
  // The exception is converted to a JavaScript Error object.
  // Has no effect if the promise is already resolved or rejected.
  void reject(Lock& js, kj::Exception&& exception) {
    state->reject(js, kj::mv(exception));
  }

  // Reject the promise with a jsg::Value (wrapped JS value).
  // Has no effect if the promise is already resolved or rejected.
  void reject(Lock& js, Value&& error) {
...
class DeferredPromise {
 public:
  ...
  // Uses direct state construction to avoid creating Pending state + transition.
  static DeferredPromise rejected(Lock& js, v8::Local<v8::Value> exception) {
    return DeferredPromise(
        kj::rc<_::DeferredPromiseState<T>>(_::DirectRejectedTag{}, Value(js.v8Isolate, exception)));
  }

  static DeferredPromise rejected(Lock& js, kj::Exception&& exception) {
    return rejected(js, js.exceptionToJs(kj::mv(exception)).getHandle(js));
  }

  static DeferredPromise rejected(Lock& js, Value&& error) {
    return rejected(js, error.getHandle(js));
  }
  ...
  //
  // The error handler receives (Lock& js, jsg::Value error) and must return
  ...
  auto then(Lock& js, Func&& func, ErrorFunc&& errorFunc)
    ...
    static_assert(kj::isSameType<Output,
                      RemoveDeferredPromise<RemovePromise<ReturnType<ErrorFunc, Value, true>>>>(),
  ...
  // Get a void promise that resolves when this promise settles.
  // DOES NOT CONSUME the promise - you can still call .then() after this.
  // Always resolves (never rejects), even if the original promise rejects.
  // Useful for waiting on completion without caring about the value.
  DeferredPromise<void> whenResolved(Lock& js) {
    ...
    KJ_SWITCH_ONEOF(state->state.underlying()) {
      KJ_CASE_ONEOF(pending, Pending) {
        // Create a new void promise that will be resolved when this one settles
        ...
        pending.resolutionObservers.add(
            [rs = kj::mv(resultState)](Lock& js, bool succeeded) mutable {
          if (succeeded) {
            rs->resolve(js);
          } else {
            // For whenResolved, we just resolve even on rejection
            // (the caller doesn't care about the error, just that it settled)
            rs->resolve(js);
          }
        });
      ...
      KJ_CASE_ONEOF(rejected, Rejected) {
        // Already rejected - but whenResolved still resolves (not rejects)
        return DeferredPromise<void>::resolved();
      }
  ...
  Promise<T> toJsPromise(Lock& js) {
    ...
    KJ_SWITCH_ONEOF(state->state.underlying()) {
      ...
      KJ_CASE_ONEOF(rejected, Rejected) {
        // Extract error before transition since reference becomes invalid
        // Note: forceTransitionTo needed because ErrorState makes Rejected implicitly terminal
        auto error = kj::mv(const_cast<Rejected&>(rejected).error);
        state->state.template forceTransitionTo<Consumed>();
        return js.rejectedPromise<T>(error.getHandle(js));
      }
      KJ_CASE_ONEOF(consumed, Consumed) {
        KJ_FAIL_REQUIRE("DeferredPromise already consumed");
      }
  ...
  DeferredPromise<Output> thenImpl(Lock& js, Func&& func, ErrorFunc&& errorFunc) {
    ...
    KJ_SWITCH_ONEOF(state->state.underlying()) {
      KJ_CASE_ONEOF(pending, Pending) {
        ...
        if constexpr (isVoid<T>()) {
          pending.thenCallback = [f = kj::mv(func), rs = kj::mv(resultState)](Lock& js) mutable {
            try {
              ...
              rs->reject(js, js.exceptionToJs(kj::getCaughtExceptionAsKj()));
            } catch (...) {
              rs->reject(js, kj::getCaughtExceptionAsKj());
            }
          };
        } else {
          pending.thenCallback = [f = kj::mv(func), rs = kj::mv(resultState)](
        ...
        pending.catchCallback = [ef = kj::mv(errorFunc), rs = resultStateRef.addRef()](
                                    Lock& js, Value error) mutable {
          try {
            if constexpr (isVoid<RawOutput>()) {
              ef(js, kj::mv(error));
              rs->resolve(js);
            } else {
              auto result = ef(js, kj::mv(error));
              resolveWithChaining<RawOutput, Output>(js, rs, kj::mv(result));
            }
          } catch (JsExceptionThrown&) {
            rs->reject(js, js.exceptionToJs(kj::getCaughtExceptionAsKj()));
      ...
      KJ_CASE_ONEOF(resolved, Resolved) {
        ...
        if constexpr (isVoid<T>()) {
          ...
          try {
            ...
            return DeferredPromise<Output>::rejected(
                js, js.exceptionToJs(kj::getCaughtExceptionAsKj()));
          } catch (...) {
            return DeferredPromise<Output>::rejected(js, kj::getCaughtExceptionAsKj());
          }
        } else {
          auto value = kj::mv(const_cast<Resolved&>(resolved).value);
          state->state.template transitionTo<Consumed>();
      ...
      KJ_CASE_ONEOF(rejected, Rejected) {
        ...
        // Extract error before transition since the reference becomes invalid after
        // Note: forceTransitionTo needed because ErrorState makes Rejected implicitly terminal
        auto error = kj::mv(const_cast<Rejected&>(rejected).error);
        state->state.template forceTransitionTo<Consumed>();
        try {
          if constexpr (isVoid<RawOutput>()) {
            errorFunc(js, kj::mv(error));
            return DeferredPromise<Output>::resolved();
          } else {
            auto result = errorFunc(js, kj::mv(error));
            if constexpr (kIsDeferredPromise<RawOutput>) {
              return kj::mv(result);
            } else if constexpr (kIsJsgPromiseType<RawOutput>) {
              auto resultState = kj::rc<_::DeferredPromiseState<Output>>();
  ...
  DeferredPromise<Output> thenImplNoError(Lock& js, Func&& func) {
    ...
    KJ_SWITCH_ONEOF(state->state.underlying()) {
      KJ_CASE_ONEOF(pending, Pending) {
        ...
        // No error handler - propagate rejection
        pending.catchCallback = [rs = resultStateRef.addRef()](Lock& js, Value error) mutable {
          rs->reject(js, kj::mv(error));
        };
      ...
      KJ_CASE_ONEOF(rejected, Rejected) {
        ...
        auto error = kj::mv(const_cast<Rejected&>(rejected).error);
        state->state.template forceTransitionTo<Consumed>();
        return DeferredPromise<Output>::rejected(js, error.getHandle(js));
      }
      KJ_CASE_ONEOF(consumed, Consumed) {
        KJ_FAIL_REQUIRE("DeferredPromise already consumed");
      }
        ...
        pending.catchCallback = [ef = kj::mv(errorFunc), rs = resultStateRef.addRef()](
          ...
          try {
            if constexpr (isVoid<T>()) {
              ...
              rs->resolve(js, ef(js, kj::mv(error)));
            }
          } catch (JsExceptionThrown&) {
            rs->reject(js, js.exceptionToJs(kj::getCaughtExceptionAsKj()));
          } catch (...) {
      ...
      KJ_CASE_ONEOF(rejected, Rejected) {
        ...
        try {
          if constexpr (isVoid<T>()) {
            ...
            return DeferredPromise<T>::resolved(errorFunc(js, kj::mv(error)));
          }
        } catch (JsExceptionThrown&) {
          return DeferredPromise<T>::rejected(js, js.exceptionToJs(kj::getCaughtExceptionAsKj()));
        } catch (...) {
          return DeferredPromise<T>::rejected(js, kj::getCaughtExceptionAsKj());
        }
      }
      KJ_CASE_ONEOF(consumed, Consumed) {
        KJ_FAIL_REQUIRE("DeferredPromise already consumed");
